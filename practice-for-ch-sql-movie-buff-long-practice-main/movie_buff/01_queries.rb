...
def harrison_ford
  ...
  # as a lead actor.
  Actor.select('movies.id', 'movies.title').joins(:movies).where("name = 'Harrison Ford' AND castings.ord != 1")
...
def biggest_cast
  ...
  # actors).
  Actor.select('movies.id', 'movies.title').joins(:movies).group('movies.id').order('COUNT(movies.id) DESC').limit(3)
end
